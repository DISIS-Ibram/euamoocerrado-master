...
Raphael.fn.fitContent = function(){
   ...
   var setAll = this.set();
    ...
    var b = setAll.getBBox();
    this.setViewBox(b.x,b.y,b.x2-b.x,b.y2-b.x);
}
...
//fn para clonar o set com os grupos
 Raphael.st.cloneSet = function () {

        var groupsSet = {};
        oset = this;
        paper = this[0].paper;
        var me_set = paper.set();
        //
        for (setAtual in oset.groups ){
           //set = s.groups[j];
             groupId = setAtual.toString();
             s = paper.set();
             for (var i = 0, ii = oset.groups[setAtual].items.length; i < ii; i++) {
                el = oset.groups[setAtual].items[i].clone()
                 s.push(el);
                 me_set.push(el);
...
//fn para clonar o set com os grupos
 Raphael.st.cloneSetToPaper = function (paperDest) {
...
//aplico transformacoes reais ao o set
 Raphael.st.position = function(deg) {
          //pego o boundbox;
          var bb = this.getBBox(true);
          var cX = bb.x+bb.width/2;
          var cY = bb.y+bb.height/2;
         // this.transform("...R"+deg+" "+cX+" "+cY);
          return {x:cX,y:cY};
 ...
 Raphael.st.stop  = function(){

    set.tween.pause();

 }

 Raphael.st.anima = function(fps){
        fps = fps || 7;
        ...
        for(grp in this.groups){
          if(/FRAME/.test(grp))
              frames.push(this.groups[grp]);
        }

        if(frames.length == 0) return;

        set.frames = frames;
        set.totalframes = frames.length - 1;
        totalframes = set.totalframes;
        set.frame = 0;
        set.prev_frame = 0;

        if(set.tween) TweenMax.killTweensOf(set);
        ...
        var time = totalframes/fps;
       VTL_TIMELINE.add(set.tween.to(set, time, {
            frame: totalframes,
            onUpdateScope: set,
            onUpdate: function() {
                ...
                try {
                    if (this.frame != this.prev_frame) {
                        svgobj = this.frames[this.prev_frame];
                        svgobj.hide();
                    }
                    svgobj = this.frames[this.frame];
                    svgobj.show();
                } catch (e) {
                }
...
//aplico transformacoes reais ao o set
 Raphael.st.selfRotate = function(deg) {
          //pego o boundbox;
          var bb = this.getBBox(true);
          var cX = bb.x+bb.width/2;
          var cY = bb.y+bb.height/2;
...
//aplico transformacoes reais ao o set
 Raphael.st.applyTransform = function () {

          this.forEach(function(e){
              var tr = e.transform().toString();
              var pt = e.attr('path').toString();
              e.attr({'path':Raphael.transformPath(pt,tr)});
              e.transform("");
          });
...
//aplico transformacoes reais aos paths
 Raphael.el.getPoints = function () {
              e = this;
              var tr = e.transform().toString();
              var pt = e.attr('path').toString();
             // e.attr({'path':Raphael.transformPath(pt,tr)});

              var tam = e.getTotalLength();
              var points = [];

              for(var i = 0; i<tam; i += tam/20){
                  var p = e.getPointAtLength(i);
                  points.push({x:p.x,y:p.y,rotation:p.alpha});
              }

              var pointsOptimizado = [];
              var pointRef = points[0];
              pointsOptimizado.push(pointRef);

              for (var i = 1; i < points.length; i++) {
                    var pp = points[i];
                    if(Math.sqrt((pointRef.x - pp.x)*(pointRef.x - pp.x)+(pointRef.y - pp.y)*(pointRef.y - pp.y)) > 100 ){
                        pointsOptimizado.push(pp);
                        pointRef = pp;
                    }
              };

              e.attr({'path':pt});
              return pointsOptimizado;
};
...
//aplico transformacoes reais aos paths
 Raphael.el.applyTransform = function () {
              e = this;
              var tr = e.transform().toString();
              var pt = e.attr('path').toString();
              e.attr({'path':Raphael.transformPath(pt,tr)});
 ...
 Raphael.el.showProgressive = function(duracao){

  //crio uma pathguid temporarimente
    var pathstr = this.attr('path').toString();
    var canvas = this.paper;
    var guide_path = canvas.path( pathstr ).attr( { stroke: "none", fill: "none" } );

    //o caminho
    var path = this.attr({'path':guide_path.getSubpath( 0, 1 )});
    path.anima = 0;
    path.guia = guide_path;

    var total_length = guide_path.getTotalLength( guide_path );

    TweenMax.to(path,(duracao/1000),{anima:total_length,onUpdateScope:path,onUpdate:function(){
        novocaminho = this.guia.getSubpath(0,this.anima);
        this.attr({path:novocaminho});
    }});
...
// //fn para clonar o set com os grupos
 Raphael.fn.cloneSet = function (oset) {
         var groupsSet = {};
         var me_set =this.set();
         //
         for (setAtual in oset.groups ){
...
// //fn para clonar o set com os grupos
 Raphael.fn.cloneSetToPaper = function (oset, desPaper) {
         var groupsSet = {};
         var me_set = desPaper.set();
         //
         for (setAtual in oset.groups ){
            //set = s.groups[j];
              groupId = setAtual.toString();
...
Raphael.fn.addGuides = function() {
 ...
 // if ( point.alpha > 360 )  point.alpha -= 360;
   ...
   // if ( point.alpha > 180 )  point.alpha -= 360;
    ...
    speed_angle = (percent*(150*360)) * Math.PI / 180;

    speed_x = point.x+(speed_length * Math.cos(speed_angle));
    speed_y = point.y+(speed_length * Math.sin(speed_angle));

    var t = {
      transform: "t" + speed_x + " " + speed_y
    };

    console.log(point.alpha+" --  "+speed_angle)

  //  $("#conteudo").css({left:-point.x+600,top:-point.y+00+(1933/2)});
