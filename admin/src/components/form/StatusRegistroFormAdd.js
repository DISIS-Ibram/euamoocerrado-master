...
class StatusRegistroFormAdd extends React.Component {
  ...
  render() {
    ...
    return (
      <form onSubmit={handleSubmit} className={className}  onChange={(e)=>this.onFormChange(e)} > 
        <div>
          <Field className="nome ui input" autoComplete="off" component="input" name="nome" placeholder={` + Adicionar ${this.props.label}`}  type="text" value=""/>
        </div>
        <div>
          <Field className="autoheight descricao" rows="1" autoComplete="off" name="descricao" placeholder="Descrição" component="textarea"  type="text"/>
        </div>
...
StatusRegistroFormAdd = reduxForm({
...
connect( 
  ...
  si3Actions)(StatusRegistroFormAdd);

export default StatusRegistroFormAdd;