...
import { Campo } from "components/formfields"; // Erro PropTypes
...
export default class Formulario extends Component {
    ...
    campos = _.map(opcoes, (v, k) => {
      ...
      switch (v.type.toLowerCase()) {
        case "string":
          ...
          if (k == "link") {
            //pq a api tras um max_length muito grande para o link
            tipo = "input";
          }
      ...
      if (k != "id") {
        ...
        if (!_.find(this.camposEncontrados, (v) => v == k)) {
          return this.criaValidacaoEtcNoCampo(
            <Campo
              key={k}
              label={label}
              name={k}
  ...
  criaValidacaoEtcNoCampo = (elm) => {
    ...
    if (_.has(model, "form.hidden")) {
      if (_.some(model.form.hidden, (v) => v === elm.props.name)) {
        newPropsForCampos["className"] =
          _.get(elm, "props.className") + " hidden";
      }
    }
  ...
  render() {
    ...
    const { invalid, submitFailed, handleSubmit, pristine, reset, submitting } =
      this.props;
    const editing = this.props.id == 0 ? "new" : "edit";

    let className =
      this.props.tipo == "small" ? "formulario-pequeno" : "formulario";
    ...
    return (
      <div
        className={
          ...
          editing +
          " " +
          (submitFailed && !submitting && " submitfailed") +
          " " +
          (invalid && "com-erro")
        }
      >
        ...
        <div className="campogrupo">
          ...
          {this.props.autoForm && this.criaCamposApi()}
        </div>

        {this.props.butoesOnPristine === false &&
          this.props.tipo != "popup" && (
            <div className="row">
                ...
                <Button.Group className="botoes">
                  {!pristine && (
                    <div>
                      ...
                      >
                        <Icon name="check" />
                        {(submitting && this.props.salvandoLabel) ||
                          this.props.salvarLabel}
                      </Button>
                  ...
                  {pristine && (
                    <div>
                      <Button type="submit">
                        <Icon name="check" />
                        {(this.props.id == 0 && this.props.criarLabel) ||
                          this.props.salvoLabel}
                      </Button>
                    </div>
                  )}
                </Button.Group>
