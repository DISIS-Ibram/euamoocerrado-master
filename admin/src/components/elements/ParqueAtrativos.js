...
import { Map, Marker, Tooltip, Popup, TileLayer, LayersControl, FeatureGroup, Circle, CircleMarker, GeoJSON, Polygon } from 'react-leaflet';
...
export  default class MapaAtrativosDefault extends React.Component {
  ...
  static defaultProps = {
    modelo:["tipoatrativo","tipobenfeitoria"],
    id:"all",
    idparque:0,
  //   options:{includes:['benfeitoria_set']},
    loader:true,
  ...
  render(){
      ...
      let itemsAtrativos = []
      let itemsBenfeitorias = []
      let items = []

      _.each(this.props.itens.tipoatrativo, item=>{
        itemsAtrativos.push( {...item, _tipo:'atrativo', _tipoGeom:'point' , _identifier:item.id+'tipoatrativo'} )
      })
      _.each(this.props.itens.tipobenfeitoria, item=>{
        itemsBenfeitorias.push( {...item, _tipo:'benfeitoria',  _tipoGeom:'point',  _identifier:item.id+'tipobenfeitoria'} )
      })

      items = itemsAtrativos.concat(itemsBenfeitorias)

      const atrativosById = _.keyBy(itemsAtrativos,'id')
      const benfeitoriasById = _.keyBy(itemsBenfeitorias,'id')
      ...
      return (
        ...
        <div className="row">
              <div className="col-xs-12">
                ...
                <h3> Atrativos e Benfeitorias vb </h3>

                <p >
                  <small>
                       Selecione um atrativo que deseja incluir e depois clique on deseja incluir no mapa abaixo
                    </small>
                ...
                <div className="row">
                  ...
                  <div className='row'> 
                              { items.map( item => 
                                    <div key={item._identifier}
                                      onClick={e=>this.setState({selected:item})}
                                      style={{    display:'inline-block','width':'60px', height:'60px', marginRight:'2px', marginBottom:'2px', cursor:'pointer', padding:'10px', background:(_.get(this.state.selected,'_identifier')==item._identifier && 'lightblue' || 'white'), color:'black', textAlign:'center'}} >
                                      <img style={{width:'25px',maxHeight:'25px'}} src={item.icone}/>
                                      <div style={{fontSize:'8px', lineHeight:'7px'}}>{item.nome}</div>
                                    </div> 
                ...
                <MapaAtrativos id={this.props.idparque} 
                    tipobenfeitoria={this.props.itens.tipobenfeitoria} 
                    tipoatrativo={this.props.itens.tipoatrativo} 
                    currentAtrativo={this.state.selected}
                    atrativosById={atrativosById}
                    benfeitoriasById={benfeitoriasById}
                    />
...
export  class MapaAtrativos extends React.Component {
    ...
    componentDidMount() {
      ...
      if(this.refs.map){
        setTimeout(()=>{
            this.refs.map.leafletElement.fitBounds(this.refs.featured.leafletElement.getBounds())
        },400)
        ...
        setTimeout(()=>{
          ...
          if(featureGroup){
            ...
            featureGroup.on('click',(e)=>{
                if(e.layer.options.model === "atrativo"){
                  let atrativoid = e.layer.options.item.id

                       this.props.openModal( { nome:"atrativo",
                            tipo:'form',
                            modelo:"atrativo",
                            onSave:()=>{
                              this.props.load('parque',{},{id:this.props.id})
                           },
                           id:atrativoid,
                           value:{}
                          } 
    ...
    componentDidUpdate(prevProps, prevState){
        ...
        if(this.refs.map){
          ...
          setTimeout(()=>{
            ...
            try{
                if(document.querySelector(".leaflet-draw-draw-marker"))
        ...
        if(this.props.input.value != ''){  
          ...
          util.geo.getAreasName( this.props.input.value ,
              (end)=>{
                  ...
                  this.setState({display: "Área: "+areatotalstr+ " -  TIS: "+ this.locationvalue });
              }, this)  
...
saveParqueGeom = async()=>{
      ...
      let geojson = {type:"Feature",geometry:{
        ...
        let res = await this.props.save('parque', { id:this.props.id, geom } )
        console.log("salvou")
        console.log(res)
  ...
  if(! _.isEmpty(markerLayers)){
    markerLayers.forEach(async layer=>{
        ...
        let res = await this.props.save(model, { id:item.id, geom:item.geom } )
        console.log("salvou")
        console.log(res)
    })
  }
 ...
 processFiles = (filelist)=>{
        ...
        //processo o json
          ...
          .then((data)=> {
            ...
            this.setState({modoExibicao:'selecao',isFetchingGeo:false,uploaded_GeoJSON:data,geogsonid:(this.state.geogsonid+1)});
          })
    ...
    render (){
      ...
      if(this.props.currentAtrativo){   
        marker = {
            repeatMode:true,
            icon: L.icon({
              iconUrl: window.SI3CONFIG.url+"obtericone/?url="+this.props.currentAtrativo.icone,//_.get(item,'[0].tipo_benfeitoria.icone',''),
              iconSize: [30, 30],
              iconAnchor: [15, 15],
              popupAnchor: [-3, -76],
            })
      ...
      let atrativosParque = _.get(this.props,'itens.atrativo_set',[]).map( atrativo=>{ 
        let iconeurl = window.SI3CONFIG.url+"obtericone/?url="+_.get(this.props.atrativosById,`[${atrativo.tipo_atrativo}].icone`)
        if(atrativo.cor){
      ...
      return (
        ...
        <div className={'geoinput dragandrop'} ref={(el) => this.dropI = el} 
            ...
            { this.state.uploaded_GeoJSON &&
              <MapaSelectedFeature geoJSON={this.state.uploaded_GeoJSON}
                  point={false} polyline={false} polygon={true}
                          ...
                          <LayersMapa  />
                            ...
                            >                 
                              ...
                              { this.state.polygonLayers.map( item =>
                                ...
                                { (this.props.itens.benfeitoria_set || []).map( (item,i)=>
                                    //  <Marker position={[-15,-45]} />
                                    <Marker key={'benfeitoria'+i} position={[item.geom.coordinates[1], item.geom.coordinates[0] ] } 
                                    ...
                                    // }}
                                      ...
                                      model="benfeitoria"
                                      icon={ L.icon({
                                        iconUrl: window.SI3CONFIG.url+"obtericone/?url="+item.tipo_benfeitoria.icone,
                                        iconSize: [30, 30],
                                        iconAnchor: [15, 15],
                                        popupAnchor: [-3, -76],
                                      })
                                ...
                                { atrativosParque.map( (item,i)=>
                                    //  <Marker position={[-15,-45]} />
                                    <Marker key={'atrativo'+i} position={[item.geom.coordinates[1], item.geom.coordinates[0] ] }
                                      item={item}
                                      onClick={(e) => console.log('marker clicked')}
                                      onMousedown={(e) => console.log('marker clicked')}
                                      model="atrativo" 
                            ...
                            }} > Carregar Arquivo com Geometria do Parque </Button>
                            <input type="file" multiple ref="fileinput" style={{display:'none'}} onChange={(e)=>this.fileSelected(e).bind(this)}/>
