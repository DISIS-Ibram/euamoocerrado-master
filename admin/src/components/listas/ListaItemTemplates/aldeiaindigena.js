...
export const titulo = (item) => {   
    ...
    return (
        <span>
          <small className="">Indigena nº <b>{ _.get(item,'indigena.numero') } - { _.get(item,'indigena.nomeindigena_set[0].nome') } </b> </small> 
...
export const descricao = (item) => {
    return (
        <div>
                ...
                <small>   
                    { !_.isEmpty(item.data_inicial) && 
                     <span>
                         { moment(item.data_inicial).format('L') }
                    </span>
                    }
                     { !_.isEmpty(item.data_final) && 
                     <span>
                        - { moment(item.data_final).format('L') }
                    </span>
                     }
            </small>
