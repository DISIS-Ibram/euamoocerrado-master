...
const _debug = false;
const myconsole = criaconsole(_debug,' === InputSelecaoModelo | ', 'color:green;font-weight:bold')
...
class InputSelecaoModelo extends React.Component {
  ...
  constructor(props){
    ...
    if( _.has(templates, (this.props.modelo+".renderInLine") ) ){
         this.state = {...this.state,renderInLine:templates[this.props.modelo].renderInLine}
    }
    ...
    if( _.has(templates, (this.props.modelo+".rendePosition") ) ){
         this.state = {...this.state,rendePosition:templates[this.props.modelo].rendePosition}
     }
  }
  ...
  componentWillMount() {
      ...
      if(idKey)
         this.state = {...this.state,idKey:idKey}
...
//        
  ...
  createOptions = (o) => {
      ...
      if( _.has(templates, (this.props.modelo+".list") ) ){
          ...
          var nome = _.has(o, 'nome') ? o.nome : _.get(this.props.modelOptions,'name', this.props.modelo)+" "+o._id;
  ...
  removeItem=(id)=>{
      ...
      if(_.isArray(value))
        value = _.remove(value,idVal=> id!==idVal)
  ...
  renderItens=()=>{
        ...
        if(this.state.renderInLine === true)
            return null

        let selecionados;
        if(this.state.multiple){
          selecionados = _.map(this.props.input.value, (value) => _.find( this.props.itens, (v,k)=>v[this.state.idKey] == value ) )
        }else{
          selecionados= _.filter(this.props.items, item=>{
                                  let achou =_.findIndex(this.props.input.value, idSel=>idSel == item[this.state.idKey])
                                  if(achou > -1){
                                      return item
                                  }
                  })                       
        ...
        return _.map((selecionados || []), item => {
          ...
          return (<div><ItemComponent item={item} removeItem={removeItem} /></div> )
        })
  ...
  render (){
    ...
    const { req, input, label, meta: { touched, error, valid, dirty }, ...custom } = this.props;
    ...
    if(this.state.multiple == false){
       opcoes = _.concat({value:null,children:<span className='cl-cor-cinza'>---nenhum---</span>},opcoes);
    }
        ...
        <FormField {...this.props}>
            ...
            {this.state.rendePosition==="top" && this.renderItens()}
             ...
             {(this.props.multiple === false && this.props.input.value != 0) &&
              <Icon onClick={this.editaModelo} className="edit-icon" name="pencil" size="small" />
             }
...
const Adicionar = (props)=>(<Label content='Adicionar' color="gray" size="mini" icon='plus' />)
