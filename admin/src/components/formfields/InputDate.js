...
export default class InputDate extends React.Component {
    ...
    componentDidMount() {
         if( this.props.input.value != ""){
            this.setState({data:moment(this.props.input.value).format("DD/MM/YYYY")})
        }
    ...
    onChangeInputData=(e)=>{
            ...
            if (num.length < 8){
                ...
                let dataAtual = moment(num,["MM-DD-YYYY", "YYYY-MM-DD","DD/MM/YYYY","DD/MM/YY" ]);
    ...
    render (){
      ...
      if (num.length < 8){
          ...
          mesAtual = moment(dataAtualFormatada,["DD", "DD/MM" ,"DD/MM/YYYY","DD/MM/YY" ]);
          
          if(mesAtual._isValid === false)
                mesAtual = moment()
          ...
          //considero uma data formatada, tranformo em data
          let dataAtual = moment(this.props.input.value,["MM-DD-YYYY", "YYYY-MM-DD","DD/MM/YYYY","DD/MM/YY" ]);
          if(dataAtual._isValid === true){
      ...
      return (
          <FormField {...this.props} icon='calendar' dica='Digite a data no formato DD/MM/AAAA'>
             ...
             <div className="input-wrap ui input fluid f-calendario">
                 ...
                 <input       
                  ...
                  autoComplete="false"
                  autoCorrect="false"
                  onBlur={(e)=>{this.delayCloseCal(); input.onBlur()}} 
                  onChange={this.onChangeInputData}
                 />

                 <div className='ui icon icone'> { this.props.icone && this.props.icone || <i className='fa fa-calendar'></i> } </div>

                 
                  {(this.state.open && !this.props.disabled ) && 
                 
                    <div className="elemento-complemento" onClick={ ()=>{this.canceloDelayClose(); alert('a3')} } >
                        <div className=' ui calendario'
                            onClick={ ()=>{this.canceloDelayClose(); alert('a1')} }
                          >
                             <DayPicker tabindex="-10"
                                  onClick={ ()=>{this.canceloDelayClose(); alert('a2') }}
                                  numberOfMonths={1}
