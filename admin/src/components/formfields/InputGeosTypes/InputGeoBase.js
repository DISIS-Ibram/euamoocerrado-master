...
//  lidar com os drops de arquivos
//  mostrar geosgsom

// Input Date
//---------------------------
class InputGeoBase extends React.Component {
  ...
  onInputChange = (e) => {
    ...
    _.forEach(geoCoord, (o) => {
      if (o.tipo == this.geoPref) o.v += 0.2;
      return o;
    });

    //vejo quem temos a maior chance de ser
    console.log(
      `UTM: ${geoCoord[0].v} GMS:${geoCoord[1].v} GD:${geoCoord[2].v}`
    );
  ...
  checkUTM = (vl) => {
    ...
    if (vl.match(/\bZ\b\d\d\s?[NML]?/gi)) {
      //se tiver z 22 N
      c += 0.7;
    }

    if (vl.match(/\bzone\b/gi)) {
      c += 0.6;
    }

    if (vl.match(/\bzone\b|\bz\b/gi)) {
      c += 0.3;
    }
  ...
  getUTM = (vl) => {
    ...
    var zonas = vl2.match(/[z]?\b(\d\d)(?=[ns\s])\s*[ns]?/gi);
    console.log(zonas);
    ...
    //LAT E LNG in METERS
    var latlng = vl.match(/(\b\d{1,9}\.?\d*[\snswelo]{0,2}\b)/gi);
    if (latlng) {
      if (latlng.length == 2) {
        x = latlng[0];
        y = latlng[1];
  ...
  getGD = (vl) => {
    ...
    var latlng = vl.match(/([-+\s]?\d{1,3}\.?\d*[\snswelo]{0,2}\b)/gi);
  ...
  getGMS = (vl) => {
    ...
    var latlng = vl.match(
      /(?:[+-\s]|^)?\d{1,3}[°º\s]*\d{1,2}['′\s]*\d{1,2}\.?\d*["″\s]*[\snswelo]{0,2}/gi
    );
  ...
  dropdownGeos = (arr, point) => {
    ...
    _.each(arr, (o, i) => {
      ...
      if (tipo == "utm") {
        ...
        drops.push(
          this.dropdownObj(tipo, utmstr, " UTM", () =>
            this.dropdownGeosOnClick(point, "utm", utmstr)
          )
        );
      }
      if (tipo == "gd") {
        ...
        drops.push(
          this.dropdownObj(tipo, gdstr, " GD", () =>
      ...
      if (tipo == "gms") {
        ...
        drops.push(
          this.dropdownObj(tipo, gsmstr, " GMS", () =>
  ...
  render() {
    ...
    const mapmode = this.state.insertModeShift ? "insertmode" : "dragmode";
    ...
    return (
      <div className="geopointwraper rel" ref="wrappedComponent">
        <div
          className={
            "geoinput dragandrop modo-" +
            this.state.modoExibicao +
        ...
        >
          <FormField
            {...this.props}
            dica="Digite as coordenadas ou o nome do estado > municipio > localidade"
          >
                ...
                <div className="geoDropdown row-400px ">
                    ...
                    <div className="dropdown ui">
                      <div className="menu">
                        {_.map(this.state.dropdowOpcoes, (obj, i) => (
                          <div
                            className={
                              "rel pt-2 pb-2 bbd item " +
                              (this.state.selIndex == i && " selected")
                            }
                            onClick={() => {
                          ...
                          >
                            <div>
                              {obj.conteudo}
                              <div className="cl-preto04 abs psr-10px pst-0 ta-r fs09 fw100">
                                <br />
                                {obj.legenda}
                              </div>
                            </div>
                  ...
                  >
                    ...
                    >
                      ...
                      {this.state.GeoJSON && (
                        <GeoJSON
                          key={"geogjon" + this.state.geogsonid}
                          ref="geojson"
                          opacity={0.4}
                      ...
                      <FeatureGroup>
                        <DrawControl
                          ...
                          draw={{
                            rectangle:
                              (this.props.geom == "area" && true) || false,
                            circle:
                              (this.props.geom == "area" && true) || false,
                            polygon:
                              (this.props.geom == "area" && true) || false,
                            polyline: false,
                            marker: (this.props.geom != "point" && false) || {
                              icon: markers[this.props.marker],
                            },
                            edit: {
                              remove: true,
                            },
...
export class DrawControl extends LayersControl {
  static propTypes = {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      _.isEqual(this.props.draw, prevProps.draw) ||
