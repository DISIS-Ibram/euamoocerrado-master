...
const _debug = false;
const myconsole = criaconsole(_debug,' *** InputTextArea| ', 'color:orange;font-weight:bold')
...
export default class InputTextArea extends React.Component {
  ...
  componentDidMount(){
    ...
    if(this.input){
        ...
        if(this.props.richText){
          ...
          "height": 300,
          "buttons": ",,,,,,,,,,,,,,,,,,,,,video,table,link,|,align,undo,redo,\n,cut,hr,eraser,copyformat,|,fullsize"
            });
  ...
  updateHeight = ()=>{
      if(this.input){
        ...
        if(totalLinhas > 1)
            this.input.style.height = alturaFinal+20+"px";
      }
  }

  render (){
     const { req, maxChar, input, label, meta: { touched, error, valid, dirty }, ...custom } = this.props;
    ...
    //  let onBlur = this.props.onBlur || this.onBlur
     ...
     return (
        <FormField {...this.props}>
            ...
            <div className="ui fluid input input-wrap input-textarea"  style={{maxHeight:'350px'}}>        

        { !this.props.richText &&

             <textarea className='ui fluid input '
              rows="5"
              id={"input-"+input.name}
              {...input}
              {...onlyDOMProps(custom)} 
