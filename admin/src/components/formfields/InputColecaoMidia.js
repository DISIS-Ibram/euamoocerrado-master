...
import { Popup, Header, Dropdown, Form, Label, List, Radio, Checkbox, Icon, Input, Dimmer, Loader, Image, Segment, Button, Accordion } from 'semantic-ui-react';
...
const _debug = false;
const myconsole = criaconsole(_debug,' === InputColecaoMidia | ', 'color:green;font-weight:bold')
...
export default class InputColecaoMidiaWrap extends React.Component {
  ...
  render(){
      ...
      return (<InputColecaoMidia modelo='colecao' {...this.props} id={id} force={true} />)
...
static defaultProps = {
  ...
  render(){
    return(
      <div>{JSON.stringify(this.props.itens)}</div>
    )
  }
}
...
class InputColecaoMidia extends React.Component {
  ...
  onChange(e) {  //chama quando clica no input que é escondido
    const { input: { onChange } } = this.props
  ...
  processFiles = (files) =>{
      ...
      if(item === false){
        ...
        this.props.save(this.props.modelo,{nome:this.props.nomeTemplate,tags:[]})
  ...
  options = [
      ...
      },
      {
        text: 'Fotos',
        value: 'Fotos',
        children: <div><Icon name="file picture outline" /> Fotos</div>,
      },
      {
        text: 'Audio',
        value: 'Audio',
        children: <div><Icon name="file audio outline" /> Audio</div>,
      },
      {
        text: 'Videos',
        value: 'Videos',
         children: <div><Icon name="file video outline" /> Videos</div>,
      },
        {
        text: 'Documentos',
        value: 'Documentos',
         children: <div><Icon name="file text outline" /> Documentos</div>,
      },
  ]
    ...
    if(item === false){
        ...
        return(
          <FormField className='ui InputColecaoMidia' {...this.props} >   
            <div className="bg-white02 h-bg-white08 t04"  
              ref={(i)=>this.dropI = i} 
              style={{display:'block'}}
                ...
                <div className="col-xs-2 ta-r">
                       <Header as='h6' className='fl-r'>
                          ...
                          <Header.Content>
                            ...
                            options={this.options} defaultValue={this.options[0].value} />
                          </Header.Content>
                        </Header>
                  </div>
              ...
              className='ui colecao-midia-drop'>
                  ...
                  {this.state.totalFiles > 0 &&
                    <div className='full zi10 row center-xs middle-xs bg-cor-laranja06'>
                      <Loader className='cl-white1' active content={`Enviando ${this.state.ultimoTotal-this.state.totalFiles+1} de ${this.state.ultimoTotal}`} inline='centered' />
                    </div>
                  }
               ...
               <MidiaList colunas={this.state.colunas} ref={(el) => this.midiacontainer = el}
                ...
                <div className="mt1 toolbar row">
                  ...
                  <div className=' upload float-right ta-r col-xs-6'>
                       <i onClick={() => this.fileInput.click()} className='fs2 op04 t02 h-cl-ae1 h-op1 fa fa-cloud-upload' />
                  </div>
                ...
                <Accordion className='mt--2' >
                  <Accordion.Title> <Label> Detalhes da coleção </Label> </Accordion.Title>
                   <Accordion.Content> 
                    <ColecaoForm 
                            id={colecao.id}
    ...
    return(
      <div ref="midiaContainer" 
          ...
          {_.map(midias, (m,k) =>
            ...
            <div key={'midia'+m.id}  style={{width:sizeCol}} className='midia-col'>
              <MidiaItem 
...
class ColecaoForm extends React.Component {
 ...
 render(){
    return (
        <Formulario tipo="small"  {...this.props} tipo="placeholder mini" butoesOnPristine={true}>
            <Campo tipo="texto" name="nome"  label="Nome" valueInvisible={this.props.nomeTemplate} />
 ...
 render(){
    ...
    if(this.props.midia.tipo_midia == 0)
          ...
          MidiaRender = (
              <div className='midia-preview' onClick={this.toogle}>
                  <div className='fundo' style={{backgroundImage:'url('+util.thumb(this.props.midia.arquivo,200)+')'}} >
                  </div>
                  <div className='fundo hover' style={{backgroundImage:'url('+util.thumb(this.props.midia.arquivo,200)+')'}} >
                  </div>
        ...
        <div className='midia-preview video' onClick={this.toogle}>
            <div className='fundo bg-cor-bege row middle-xs center-xs' >
                 <div className='col-xs-11'>
                  <div><i className='fa fa-file-video-o fa-2x'></i></div>
    ...
    MidiaRender = (
        <div className='midia-preview audio' onClick={this.toogle}>
            <div className='fundo bg-cor-bege row middle-xs center-xs ' >
                <div className='col-xs-11'>
                  <div><i className='fa fa-file-audio-o fa-2x'></i></div>
                  <div className='ta-c fs07 truncate'>{this.props.midia.nome}</div>
                </div>
    ...
    MidiaRender = (
        <div className='midia-preview documento' onClick={this.toogle}>
            ...
            {this.props.midia.arquivo.toLowerCase().indexOf('.pdf') > -1 &&
              ...
              <div className='fundo row middle-xs center-xs ' style={{backgroundImage:'url('+util.thumb(this.props.midia.arquivo,200)+')'}} >
                 <div className='col-xs-11 icone-com-thumb'>
                    <div><Label size="mini" color="orange"><i className='fa fa-file-pdf-o fa-2x'></i></Label></div>
            ...
            ||
              <div className='fundo row middle-xs center-xs '>
                <div className='col-xs-11'>
                    <div><i className='fa fa-file-text-o fa-2x'></i></div>
                    <div className='ta-c fs07 truncate'>{this.props.midia.nome}</div>
                  </div>
              </div>
            }
    ...
    if(this.props.listStyle){
          ...
          MidiaRender =  (
                <div  onClick={this.toogle} className='row h-bg-a02 middle-xs col-xs-12 bbd'>
                   
                   <div className='col-xs-2 mt-05 mb-05' style={{width:this.props.size}}>
                   {MidiaRender}
                   </div>
                   
                   <div className='col-xs mr10'>
                        <b className='cl-ae'>{this.props.midia.nome}</b><br/>
                        {this.props.midia.autor != undefined &&
                          <small><i>Autor:{this.props.midia.autor}</i><br/></small>
                        }
                        {this.props.midia.data != undefined &&
                          <small><i>Data:{this.props.midia.data}</i><br/></small>
                        }
      ...
      var conteudo=( <div ref={ (e)=>this.detail = e } style={{width:'400px'}}>
    ...
    return (
      ...
      <div          
          ...
          className={'midia-form '+ (this.state.open && ' open ') + ' ' + (this.props.listStyle && ' lista ')} >      
              ...
              ()=> {
                ...
                try{
                    ...
                    if (left < 60){
                      let dif = 70-left
                      left = 70;
                      $(this.detail).find('.seta').css('marginLeft',(-dif+'px'))
                    }
                    ...
                    if((fb < pt) || (ft > pb) ){
                        ...
                        $(this.detail).css({display:'block',top:top+'px',left:left+'px'})
                    }
...
class MidiaForm extends React.Component {
    ...
    remove = ()=>{
        ...
        var ok = confirm("Deseja apagar o Arquivo '"+this.props.itens.nome+"'?");
                  ...
                  <div className="row middle-xs mt--1"> 
                       ...
                       <div className='col-xs-12'>
                         <Button.Group className='fl-l' basic size='small'>
                          <Button as="a" href={midia.arquivo} target="_blank" className="cl-w h-bg-b05" icon='search' />
                          <Button as="a" className="cl-w h-bg-b05" icon='download' href={window.SI3CONFIG.downloadMidia+midia.id} />
                          <Button as="a" onClick={this.remove} className="cl-b06 h-bg-b05" icon='trash' />
                         </Button.Group>
                  ...
                  <div className="row col-xs-12 mt-05">
                        ...
                        {midia.tipo_midia == 2 &&
                            <ReactPlayer 
                              width="100%"
                              height="40px"
                              url={midia.arquivo} controls={true} playing />
                        }
                        ...
                        {midia.tipo_midia == 0 &&
                            <div style={{position:'relative',width:'100%',height:'0',paddingBottom:'56%'}}>
                             <img src={midia.arquivo} style={{maxHeight:'100%',maxWidth:'100%'}} />
                              <div className='full bg-contain' style={{backgroundImage:'url('+midia.arquivo+')'}} >
                        </div>
                            </div>
                        }

                        {(midia.tipo_midia == 3 && midia.arquivo.toLowerCase().indexOf('.pdf')) &&
                          <div style={{position:'relative',width:'100%',height:'0',paddingBottom:'56%'}}>
                             <img src={util.thumb(midia.arquivo,600)} style={{maxHeight:'100%',maxWidth:'100%'}} />
                             <div className='full bg-contain'  style={{backgroundImage:'url('+util.thumb(midia.arquivo,600)+')'}} >
                            </div>
                          </div>
                        }
                    ...
                    <div className="row center-xs col-xs-12">
                      <div className="col-xs-12 col-sm-12 col-lg-10">
                       <div className='virtualForm formulario formulario-pequeno formulario-invertido'>
                        <Formulario tipo="small placeholder mini"  {...this.props} butoesOnPristine={true}>
                            <Campo tipo="texto" name="nome"  label="Nome" />
                            <Campo tipo="texto" name="autor" label="Autor" />
                            <Campo tipo="textoarea" name="descricao" label="Descricao"  />
                            <Campo tipo="data" name="data" label="Data"  />
                            <Campo tipo="point" name="geom" label="Localização"  />
                            <Campo tipo="modelo" modelo="tag" multiple name="tags" label="Tags"  />
                        </Formulario>
                       </div>
                      </div>
                    </div>
