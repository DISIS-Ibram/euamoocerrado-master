...
class LayersMapa extends React.Component {
  ...
  renderLayer = (obj) => {
    ...
    if (obj.tipo_mapa == 0) {
      return (
        <LayersControl.BaseLayer key={obj.nome} name={obj.nome} checked={chk}>
          <WMSTileLayer
      ...
      return (
        <LayersControl.Overlay key={obj.nome} name={obj.nome} checked={chk}>
          <WMSTileLayer
            attribution={`&copy; <a href="${obj.link}">${obj.creditos}</a> contributors`}
            url={obj.url}
            layers={obj.layer}
  ...
  render() {
    ...
    return (
      <LayersControl position="topright">
        {_.map(
          _.sortBy(layers, (o) => o.tipo_mapa),
          (elm) => this.renderLayer(elm)
        )}
      </LayersControl>
    );
  }
