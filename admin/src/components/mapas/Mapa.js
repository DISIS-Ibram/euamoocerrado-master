...
class Mapa extends React.Component {
  static defaultProps = {
    ...
    modelos: [
      ...
      "aldeia",
      "habitacao",
    ], //lista de modelos que devo receber
    //modelos: ['relato'],
      ...
      features: [
        {
          ...
          geometry: {
            type: "LineString",
            coordinates: [
              [-51.78955078125, -13.902075852500483],
              [-54.29443359375, -15.347761924346937],
            ],
          },
        },
        {
          ...
          geometry: {
            type: "LineString",
            coordinates: [
              [-51.5478515625, -14.200488387358332],
              [-50.38330078125, -16.088042220148807],
            ],
          },
        },
        {
          ...
          geometry: {
            type: "LineString",
            coordinates: [
              [-51.52587890625, -13.966054081318301],
              [-47.79052734375, -13.88074584202559],
            ],
          },
        },
      ],
  ...
  carregaModelosBusca = (geo = "", options = {}) => {
    ...
    var stateModelos = {};
    _.each(this.props.modelos, (k) => {
      stateModelos[k] = false;
    });
    ...
    //limpo as linhas
    this.setState({
      linhasRelacoesGeoJSON: { type: "FeatureCollection", features: [] },
    });
    ...
    _.each(this.props.modelos, (modelo) => {
      si3.getGeoJson(modelo, optionsFinal).then((data) => {
        if (!_.isEmpty(this.state.filter)) {
          ...
          _.each(data.features, (feature) => {
            ...
            var linefeature = turf.lineString(
              [
                [this.state.popupLocation[1], this.state.popupLocation[0]],
                pointdest.geometry.coordinates,
              ],
              { name: "line 1" }
            );
          ...
          var final = [
            ...this.state.linhasRelacoesGeoJSON.features,
            ...features,
          ];
  ...
  polyCreated = (e) => {
    ...
    if (this.layerantigo) this.layerantigo.remove();
  ...
  render() {
    ...
    return (
      <div
        className={`mapwrap col-xs-12 row-xs-12`}
      ...
      >
        ...
        >
              ...
              (modelo) =>
                _.isArray(_.get(this.state, modelo + ".features")) && (
                  <LayersControl.Overlay
                    key={"geooverlay" + modelo}
                    name={
                      modelo +
                      "(" +
                      _.get(this.state[modelo], "features.length", 0) +
                      ")"
                    }
                    checked={true}
                  >
                    <GeoJSON
                      ...
                      pointToLayer={(f, l) => {
                        return L.marker(l, {
                          icon: markersMap(modelo, f.properties),
                        });
                      }}
          ...
          <GeoJSON
            key={this.state.linhasRelacoesGeoJSON.features.length}
            ref="linhasReferencia"
            opacity={1}
            color="white"
            stroke="white"
            data={this.state.linhasRelacoesGeoJSON}
          />
            ...
            {this.state.geoTi && (
              <LayersControl.Overlay name="Terras Indigenas" checked={true}>
                <GeoJSON
                  ...
                  ref="geojsonti"
                  opacity={0.4}
                  data={this.state.geoTi}
                  // onClickNO={(e) => this.onGeoJsonClick(e, "terra_indigena")}
                  onClick={(e) => this.onGeoJsonClick(e, "terra_indigena")}
                  fillColor="red"
                  color="red"
                />
              </LayersControl.Overlay>
            ...
            {this.state.geoEstados && (
              ...
              >
                <GeoJSON
                  ...
                  color="blue"
                  opacity={0.4}
                  data={this.state.geoEstados}
                  // onClickNO={(e) => this.onGeoJsonClick(e, "estados")}
                  onClick={(e) => this.onGeoJsonClick(e, "estados")}
                />
              </LayersControl.Overlay>
            )}
            {this.state.geoMunicipio && (
              <LayersControl.Overlay name="Municipio" checked={false}>
                <GeoJSON
                  ...
                  color="orange"
                  opacity={0.4}
                  data={this.state.geoMunicipio}
                  // onClickNO={(e) => this.onGeoJsonClick(e, "municipio")}
                  onClick={(e) => this.onGeoJsonClick(e, "municipio")}
                />
              </LayersControl.Overlay>
            )}
          </LayersControl>
          ...
          {this.state.popupModelo != false && (
            <Popup
              ...
              minWidth={300}
              offset={[0, -46]}
              key={"loc" + this.state.popupLocation[0]}
              position={this.state.popupLocation}
            ...
            >
              <span>
                <AutoView
                  store={window.STORE}
                  modelo={this.state.popupModelo}
                  id={this.state.popupId}
                />
                <div>
                  <a
                    href={`/form/${this.state.popupModelo}/${this.state.popupId}`}
                  >
                    {" "}
          ...
          <FeatureGroup>
            <DrawEditControl
              position="topleft"
              onCreated={this.polyCreated}
              onEdited={this.polyCreated}
              draw={{
                rectangle: true,
                circle: false,
                polygon: true,
