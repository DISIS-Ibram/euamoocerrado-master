...
//a new search items
export const filterItens = async (
  items,
  keyword = "",
  propriedadesAProcurar = [],
  idKey = false,
  optionsExtend = {}
) => {
  ...
  //     options.id = idKey;
  myconsole.log(
    "FUSE SEARCH pavara:%s itens: %o, OPTIONS %o, keys: %o ",
    keyword,
    items,
    options,
    keys
...
export const serializeModel = (obj, tipo, entities) => {
  const _serializo = (
    ...
    myconsole.log(
      "SERIALIZER | Serializando %o Iniciando valor:%o, modelo:%o",
      caminho,
      valor,
      modeloTipoDoValor
    );
    ...
    myconsole.log(
      "SERIALIZER | Serializando %o... valorRetornoIntermediario:%o",
      caminho,
      valorRetorno
    );
          ...
          if (_.isArray(valorRetorno)) {
            valorRetorno = _.map(valorRetorno, (valorElemento) => {
              if (_.has(valorElemento, keyNome)) {
                ...
                valorElemento[keyNome] = _serializo(
                  valor,
                  modeloTipo,
                  entities,
                  parentObjectBag,
            ...
            if (_.has(valorRetorno, keyNome)) {
              ...
              var valorConvertido = _serializo(
                valor,
                modeloTipo,
                entities,
                parentObjectBag,
    ...
    myconsole.log(
      "SERIALIZER | Serializando %o... valorRetorno Final , valor:%o",
      caminho,
      valorRetorno
    );
    ...
    if (_.isObject(objToSerialize) && _.isArray(objToSerialize) === false) {
      ...
      if (!_.has(map, key)) {
        ...
        if (_.isString(objToSerialize) || _.isNumber(objToSerialize)) {
          ...
          parentObjectTipo.push(modelo.type);
          return _.map(objToSerialize, (id) =>
            serialize(
              entidies[modeloRefTipo][id],
              modeloRef,
              entidies,
              key,
              parentObjectTipo
...
export const normalizerModel = (obj, tipo) => {
  ...
  myconsole.log(
    "====   NORMALIZER | NORMALIZADO " + tipo.toUpperCase() + "===========%o",
    bag
  );
...
export const getGeoJson = (modelo = "terraindigena", options = {}) => {
  ...
  return $.getJSON(convergeourl, { modelo: modelo, ...options });
};
