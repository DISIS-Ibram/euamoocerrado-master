...
import { t, d, getIDKey, getIDKeyFromMap, SI3RC_MODELS } from "models/models";
...
export default function connectBaseForm(WrapComp) {
  ...
  let newClass = class BaseForm extends WrapComp {
    ...
    componentWillMount() {
      ...
      this.formNome = this.props.form
        ? this.props.form
        : this.props.modelo + "_" + this.props.id;

      this.state = { ...this.state, idAtual: this.props.id };
    }
    ...
    _prepararInfo(item) {
      ...
      if (this.props.asID == "all") {
        ...
        const pegaValueOuIdObjeto = (v, k) => {
          let vfinal = v;
          if (_.isArray(v)) {
            vfinal = _.map(v, (value) => pegaValueOuIdObjeto(value, k));
          }
          if (_.isObject(v) && !_.isArray(v)) {
            //vejo se tem um _id, quer dizer que é um objeto normalizado
            if (_.has(v, "_id")) {
              vfinal = v._id;
              //vejo se tem um id, que é padrao de objetos mesmo
            } else if (_.has(v, "id")) {
              vfinal = v.id;
              //vejo se tem algum modelo definido com esse nome e vejo qual o idkey dele e vejo se tenho nesse item
            } else {
              if (_.has(SI3RC_MODELS, k.toLocaleLowerCase())) {
                var modeloRef = SI3RC_MODELS[k.toLocaleLowerCase()];
                var idkey = _.get(modeloRef, "idkey") || "id";
                if (_.has(v, idkey)) {
                  vfinal = _.get(v, idkey);
                }
              }
            }
          }
        ...
        _.each(itemPreparado, (v, k) => {
          // if( _.isArray(v) ){
          //     var arrayValues = _.map(v,value=>pegaValueOuIdObjeto(value,k))

          //     itemPreparado[k] =  arrayValues;
          // }
          itemPreparado[k] = pegaValueOuIdObjeto(v, k);
        });
      } else {
      ...
      //e agora vejo se tem algum value determinado na url
      var valueFromUrl = _.fromPairs(
        _.compact(
          _.map(window.location.search.slice(1).split("&"), function (item) {
            if (item) return item.split("=");
          })
        )
      );

      _.each(valueFromUrl, (v, k) => {
        if (!_.has(itemPreparado, k) || _.get(itemPreparado, k) == undefined) {
          itemPreparado[k] = parseInt(v, 10) || decodeURI(v);
        }
      });

      if (_.isEmpty(itemPreparado)) {
        //o initial values precisa ser sempre um objeto, nesse caso se tiver false ou algum erro, retorno objeto vazio
    ...
    _salvar(values) {
      ...
      if (this.props.modelo == "") {
        ...
        return this.props.save(this.props.modelo, values).catch((response) => {
    ...
    render() {
      ...
      if (!this.props.itens && this.props.modelo !== "") {
        return (
          <Loader active content="Carregando formulário..." inline="centered" />
        );
      }
  ...
  newClass = connect((state, ownprops) => {
    let formNome = ownprops.form
      ? ownprops.form
      : ownprops.modelo + "_" + ownprops.id;
